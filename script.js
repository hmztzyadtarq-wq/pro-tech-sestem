...
// مصفوفة لتخزين مواعيد الصيانة
window.maintenanceList = JSON.parse(localStorage.getItem('bro_tech_maintenance')) || [];

// دالة التنقل بين صفحات النظام (تأكد أن عندك دالة مشابهة للتبويبات، ودي لتفعيل تبويب الصيانة)
window.switchTab = function(tabId) {
    ...
    if(activeTab) {
        ...
        if(tabId === 'maintenanceTab') {
            renderMaintenanceTable();
        }
    }
};

// فتح نافذة بسيطة لإضافة موعد صيانة جديد
window.openAddMaintenanceModal = function() {
    let customerName = prompt('أدخل اسم العميل:');
    if(!customerName) return;
    
    let customerPhone = prompt('أدخل رقم هاتف العميل:');
    let machineType = prompt('أدخل موديل أو نوع الماكينة (مثل: Ricoh, Xerox...):');
    let maintDate = prompt('أدخل تاريخ الصيانة القادمة (مثلاً: 2026-09-01):');
    let notes = prompt('ملاحظات أو نوع الصيانة المطلوبة (تغيير حبر، صيانة دورية...):');

    let newRecord = {
        id: Date.now(),
        name: customerName,
        phone: customerPhone || '---',
        machine: machineType || 'ماكينة طباعة',
        date: maintDate || 'غير محدد',
        notes: notes || 'صيانة دورية'
    };

    window.maintenanceList.push(newRecord);
    localStorage.setItem('bro_tech_maintenance', JSON.stringify(window.maintenanceList));
    
    // تسجيل الحدث في السجل اللي عملناه قبل كده
    if(typeof logActivity === 'function') {
        logActivity('مواعيد الصيانة', `تم تسجيل موعد صيانة جديد للعميل (${customerName}) بتاريخ ${maintDate}`);
    }

    renderMaintenanceTable();
    alert('تم تسجيل موعد الصيانة بنجاح!');
};

// عرض البيانات في الجدول
window.renderMaintenanceTable = function() {
    let tbody = document.getElementById('maintenanceTableBody');
    if(!tbody) return;

    if(window.maintenanceList.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; padding: 20px; color: #64748b;">لا توجد مواعيد صيانة مسجلة حالياً</td></tr>`;
        return;
    }

    let html = '';
    window.maintenanceList.forEach((item, index) => {
        html += `
            <tr style="border-bottom: 1px solid #334155;">
                <td style="padding: 10px;">${item.name}</td>
                <td style="padding: 10px;">${item.phone}</td>
                <td style="padding: 10px; color: #38bdf8;">${item.machine}</td>
                <td style="padding: 10px; font-weight: bold; color: #fbbf24;">${item.date}</td>
                <td style="padding: 10px; color: #94a3b8;">${item.notes}</td>
                <td style="padding: 10px; text-align: center;">
                    <button onclick="deleteMaintenance(${item.id})" style="background: #ef4444; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;"><i class="fas fa-trash"></i></button>
                </td>
            </tr>
        `;
    });
...
window.deleteMaintenance = function(id) {
    if(confirm('هل أنت متأكد من حذف موعد الصيانة هذا؟')) {
        window.maintenanceList = window.maintenanceList.filter(i => i.id !== id);
        localStorage.setItem('bro_tech_maintenance', JSON.stringify(window.maintenanceList));
        renderMaintenanceTable();
    }
};
